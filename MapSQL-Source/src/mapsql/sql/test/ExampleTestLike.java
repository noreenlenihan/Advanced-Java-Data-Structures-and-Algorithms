...
/* Test class for Task 2 (the Like Condition) to test for three cases */
public class ExampleTestLike {
...
//		showTables();
	...
	public static void insertData() {
		...
		executeStatement(
				new Insert(
						"contact", 
						new String[] {"name", "email"}, 
						new String[] {"Kevin", "kevin.codd@ucd.ie"}
				)
		);
		executeStatement(
				new Insert(
						"contact", 
						new String[] {"name", "email"}, 
						new String[] {"Noreen", "noreen.lenihan@ucd.ie"}
				)
		);
	}
	...
	/* Test method for Like condition */
	public static void selectRowsWithLike(){
		System.out.println("First result, testing % at start of string: ");
		executeStatement(new Select("contact", new String[] {"id", "name", "email"}, new Like("name", "%Reen")));
		System.out.println("Second result, testing % at end of string: ");
		executeStatement(new Select("contact", new String[] {"id", "name", "email"}, new Like("name", "kevin%")));
		System.out.println("Third result, testing substring anywhere in string (% at start and end): ");
		executeStatement(new Select("contact", new String[] {"id", "name", "email"}, new Like("name", "%Em%")));
	}
}
